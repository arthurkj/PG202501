...
int selectedTileMapLine = 5, selectedTileMapColumn = 3;
...
int main()
{
    ...
    principal.dimensions = vec3(150, 150, 1.0);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        if (selectedTileMapLine > selectedTileMapColumn)
        {
            x = x0 + (selectedTileMapLine - selectedTileMapColumn) * 57;
            y = y0 + (selectedTileMapLine + selectedTileMapColumn) * 28.5;
        }
        else
        {
            x = x0 - (selectedTileMapColumn - selectedTileMapLine) * 57;
            y = y0 + (selectedTileMapLine + selectedTileMapColumn) * 28.5;
        }
...
void key_callback(GLFWwindow *window, int key, int scancode, int action, int mode)
{
    if (action == GLFW_PRESS || action == GLFW_REPEAT)
    {
        if (key == GLFW_KEY_A)
        {
            selectedTileMapLine -= 1;
            selectedTileMapColumn += 1;
        }
        if (key == GLFW_KEY_D)
        {
            selectedTileMapLine += 1;
        ...
        selectedTileMapLine = glm::clamp(selectedTileMapLine, 1, TILEMAP_HEIGHT);
        selectedTileMapColumn = glm::clamp(selectedTileMapColumn, 1, TILEMAP_WIDTH);
    }

    std::cout << selectedTileMapLine << std::endl;
    std::cout << selectedTileMapColumn << std::endl;
    std::cout << "----" << std::endl;
}
