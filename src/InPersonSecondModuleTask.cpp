// Grupo: Arthur Kist Juchem, Felipe Eduardo Mossmann & Gustavo de Oliveira
...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        for (const auto& triangle : triangles) {
            ...
            // Escala
            model = scale(model, vec3(100.0, 100.0, 1));
...
void mouse_button_callback(GLFWwindow* window, int button, int action, int mods) {
    if (button == GLFW_MOUSE_BUTTON_LEFT && action == GLFW_PRESS)
    {
        ...
        if (vertices.size() == 3) {
            ...
            newTriangle.x = (vertices[0].x + vertices[1].x + vertices[2].x) / 3.00;
            newTriangle.y = (vertices[0].y + vertices[1].y + vertices[2].y) / 3.00;
